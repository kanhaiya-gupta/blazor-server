...
public class AASXFileServerAPIApiController : ControllerBase
{
    ...
    /// <response code="0">Default error handling for unmentioned status codes</response>
    [HttpGet]
    [Route("/packages/project/{*projectPath}")]
    [ValidateModelState]
    [SwaggerOperation("GetAASXByProjectPath")]
    [SwaggerResponse(statusCode: 200, type: typeof(byte[]), description: "Requested AASX package")]
    [SwaggerResponse(statusCode: 400, type: typeof(Result), description: "Bad Request, e.g. the request parameters of the format of the request body is wrong.")]
    [SwaggerResponse(statusCode: 401, type: typeof(Result), description: "Unauthorized, e.g. the server refused the authorization attempt.")]
    [SwaggerResponse(statusCode: 403, type: typeof(Result), description: "Forbidden")]
    ...
    [SwaggerResponse(statusCode: 500, type: typeof(Result), description: "Internal Server Error")]
    [SwaggerResponse(statusCode: 0, type: typeof(Result), description: "Default error handling for unmentioned status codes")]
    public virtual async Task<IActionResult> GetAASXByProjectPath([FromRoute] [Required] string projectPath)
    {
        _logger.LogInformation($"Received request to get the AASX Package with project path {projectPath}");
        ...
        {
            throw new NotFoundException($"AASX file not found at path: {projectPathFull}");
        }

        try
        {
            // Use the file service to load the package properly
            var fileName = _fileService.GetAASXByProjectPath(projectPath, out var content, out var fileSize, out var aas);
            
            if (fileName == null)
            {
                throw new NotFoundException($"Failed to load AASX package: {projectPath}");
            }

            // Create content disposition for download
            ContentDisposition contentDisposition = new() { FileName = fileName };
            HttpContext.Response.Headers.Append("Content-Disposition", contentDisposition.ToString());
            HttpContext.Response.Headers.Append("X-FileName", fileName);
            HttpContext.Response.ContentLength = fileSize;
        ...
        {
            _logger.LogError($"Error loading package {projectPath}: {ex.Message}");
            throw new Exception($"Failed to load AASX package: {ex.Message}");
        }
    }
