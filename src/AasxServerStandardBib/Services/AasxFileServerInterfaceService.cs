...
namespace AasxServerStandardBib.Services
{
    public class AasxFileServerInterfaceService : IAasxFileServerInterfaceService
    {
        ...
        public string GetAASXByProjectPath(string projectPath, out byte[] content, out long fileSize, out IAssetAdministrationShell aas)
        {
            aas = null;
            content = null;
            fileSize = 0;
            ...
            try
            {
                // Load the package using the new method
                var success = Program.LoadPackageByProjectPath<IAssetAdministrationShell>(out bool loadSuccess, out int packageIndex, projectPath);
                
                if (!loadSuccess || success == null)
                {
                    _logger.LogError($"Failed to load package: {projectPath}");
                    return null;
                }
                ...
                _logger.LogInformation($"Successfully loaded project file: {projectPath}");
                return filename;
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error loading project file {projectPath}: {ex.Message}");
                return null;
            }
        }
