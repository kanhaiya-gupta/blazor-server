    ...
    public static class Program
    {
        ...
        public static T? LoadPackageByProjectPath<T>(out bool success, out int packageIndex, string projectPath)
        {
            ...
            lock (changeAasxFile)
            {
                try
                {
                    Console.WriteLine($"Loading project file: {fullPath}");
                    envFileName[i] = fullPath;
                    
                    if (!withDb)
                    {
                        env[i] = new AdminShellPackageEnv(fullPath, true, false);
                        if (env[i] == null)
                        {
                            Console.WriteLine($"Cannot open {fullPath}");
                            return output;
                        }
                    }
                    else
                    {
                        VisitorAASX.LoadAASXInDB(fullPath, false, withDbFiles);
                        envFileName[i] = null;
                        env[i] = null;
                    }
                    ...
                    if (typeof(T) == typeof(IAssetAdministrationShell))
                    {
                        if (env[i]?.AasEnv?.AssetAdministrationShells?.Count > 0)
                        {
                            output = (T)(object)env[i].AasEnv.AssetAdministrationShells[0];
                        }
                    }
                    else if (typeof(T) == typeof(AdminShellPackageEnv))
                    {
                        output = (T)(object)env[i];
                    }
                ...
                {
                    Console.WriteLine($"Error loading package {fullPath}: {ex.Message}");
                }
            }
